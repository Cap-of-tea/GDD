...
$gddExe = Join-Path $scriptDir "GDD.Desktop.exe"
$baseUrl = "http://localhost:9800/mcp"
$gddProcess = $null
$gddArgs = @()
...
function Ensure-GddRunning {
    ...
    if (-not (Test-Path $gddExe)) {
        [Console]::Error.WriteLine("GDD.Desktop not found at $gddExe")
        exit 1
    }
    ...
    [Console]::Error.WriteLine("GDD.Desktop MCP server did not respond after 20s")
    exit 1
}
