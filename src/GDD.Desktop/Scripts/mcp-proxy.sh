#!/usr/bin/env bash
# stdio <-> HTTP proxy for GDD.Desktop (Avalonia GUI). Launches the GUI if not running
# and forwards MCP JSON-RPC to its HTTP server on port 9800.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
GDD_EXE="$SCRIPT_DIR/GDD.Desktop"
BASE_URL="http://localhost:9800/mcp"
GDD_PID=""
GDD_ARGS=""
for arg in "$@"; do
    case "$arg" in
...
ensure_running() {
    ...
    if [ ! -x "$GDD_EXE" ]; then
        echo "GDD.Desktop not found at $GDD_EXE" >&2
        exit 1
    fi
    ...
    echo "GDD.Desktop MCP server did not respond after 20s" >&2
    exit 1
}
...
while IFS= read -r line; do
    ...
    response=$(curl -sf -X POST "$BASE_URL" \
        ...
        response=$(curl -sf -X POST "$BASE_URL" \
            ...
            -d "$line" \
            --max-time 120 2>/dev/null) || {
            response="{\"jsonrpc\":\"2.0\",\"id\":null,\"error\":{\"code\":-32000,\"message\":\"GDD.Desktop server unreachable\"}}"
        }
    }
    echo "$response"
done
